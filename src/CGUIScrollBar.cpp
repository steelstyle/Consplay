...
void CGUIScrollBar::draw()
{
	...
	video::SColor iconColor = skin->getColor(isEnabled() ? EGDC_WINDOW_SYMBOL : EGDC_GRAY_TEXT);
...
void CGUIScrollBar::refreshControls()
{
	...
	if (skin)
	{
		...
		CurrentIconColor = skin->getColor(isEnabled() ? EGDC_WINDOW_SYMBOL : EGDC_GRAY_TEXT);
	}
